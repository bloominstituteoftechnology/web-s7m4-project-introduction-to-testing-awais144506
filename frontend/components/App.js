...
export default function App({ lang = 'en' }) {
  ...
  return (
    <div>
      <h2>
        {txt.TEXT_HEADING_CREATE_ACCOUNT}
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="inputGroup">
          <label htmlFor="username">{txt.LABEL_USERNAME}</label>
          <input id="username" name="username" onChange={onChange} value={values.username} type="text" placeholder={txt.PLACEHOLDER_USERNAME} />
          {errors.username && <div className="validation">{errors.username}</div>}
        </div>

        <div className="inputGroup">
          <fieldset>
            <legend>{txt.TEXT_FAV_LANG}</legend>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="javascript" checked={values.favLanguage == 'javascript'} />
              {txt.TEXT_FAV_LANG_JS}
            </label>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="rust" checked={values.favLanguage == 'rust'} />
              {txt.TEXT_FAV_LANG_RUST}
            </label>
          </fieldset>
        ...
        <div className="inputGroup">
          <label htmlFor="favFood">{txt.LABEL_FAV_FOOD}</label>
          <select id="favFood" name="favFood" value={values.favFood} onChange={onChange}>
            <option value="">{txt.TEXT_OPT_FAV_FOOD_1}</option>
            <option value="pizza">{txt.TEXT_OPT_FAV_FOOD_2}</option>
            <option value="spaghetti">{txt.TEXT_OPT_FAV_FOOD_3}</option>
            <option value="broccoli">{txt.TEXT_OPT_FAV_FOOD_4}</option>
          </select>
        ...
        <div className="inputGroup">
          <label>
           ...
           {txt.LABEL_ACCEPT_TERMS}
          </label>
          {errors.agreement && <div className="validation">{errors.agreement}</div>}
        </div>

        <div>
          <input type="submit" disabled={!submitAllowed} value={txt.TEXT_SUBMIT} />
        </div>
      </form>
    </div>
  )
